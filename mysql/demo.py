...
parser = argparse.ArgumentParser(description='MySQL Operations Demo Script')
parser.add_argument('--host', default='localhost', help='MySQL host')
parser.add_argument('--port', default=3306, type=int, help='MySQL port')
parser.add_argument('--user', default='mw', help='MySQL username')
parser.add_argument('--password', required=True, help='MySQL password')
parser.add_argument('--database', default='mysqldb', help='MySQL database name')
...
def run_complex_query(connection):
    ...
    if query_type == 1:
        ...
        JOIN order_items oi ON p.id = oi.product_id
        GROUP BY p.id
        HAVING total_qty > category_avg_qty
        ORDER BY (total_qty / category_avg_qty) DESC
        """)
    else:
...
def update_multiple_prices(connection):
    ...
    cursor.execute("SELECT id, name, price FROM products ORDER BY RAND() LIMIT %s", (num_to_update,))
